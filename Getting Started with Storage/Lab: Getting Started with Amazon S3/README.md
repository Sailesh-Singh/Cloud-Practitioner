...
Bucket versioning is turned off by default. When versioning is turned off, changes to objects can’t be undone. For example, if we upload a new version of a file, the old file is replaced with the new one. The original file is lost. If you delete a file, it is permanently deleted, and we can’t get it back.

However, when versioning is turned on, changed and deleted versions of files are saved. Previous versions of objects are not presented by default, but we can access them using the console or programmatically. Because we are keeping earlier versions of objects, we can recover them if we need to.

It is important to remember that once we turn on version, we cannot turn it off. However, we can suspend versioning. For more information on bucket versioning, see the Amazon Simple Storage Service Users Guide.

Recall that when we created your bucket, you turned on versioning. In this task, we view the object versions available in our bucket.
...
+   Review the list of objects in the bucket.
    ...
    +   We should also find two versions of the **index.html** file because we uploaded a new version of the file. The current version is the file that we uploaded when we updated our website.
